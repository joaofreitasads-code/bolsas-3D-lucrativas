    ...
    <div className="w-full max-w-4xl mx-auto px-4 mt-2">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {BESTSELLER_IMAGES.map((item, index) => (
          <div 
            key={index} 
            className="group relative bg-white rounded-xl border border-stone-200 shadow-md hover:shadow-xl overflow-hidden flex flex-col transition-all duration-300 hover:scale-[1.02] hover:border-[#D4AF37]/50"
          >
            {/* Landscape (Deitadas) Image Container */}
            <div className="relative w-full aspect-[16/10] overflow-hidden bg-stone-50 flex items-center justify-center">
              <img
